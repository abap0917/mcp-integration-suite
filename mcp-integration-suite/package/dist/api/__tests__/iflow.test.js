...
const index_1 = require("../iflow/index");
const index_2 = require("../packages/index"); // Need createPackage to ensure test package exists
const deployment_1 = require("../deployment"); // Need deployment status check
const diagram_1 = require("../iflow/diagram"); // Import the diagram function
...
describe("IFlow Management API", () => {
    ...
    beforeAll(async () => {
        ...
        try {
            ...
            await (0, index_2.createPackage)(testPackageId, `Jest IFlow Test Pkg`, `Temp pkg for Jest IFlow tests`);
            packageCreated = true;
    ...
    it("should create an empty iflow", async () => {
        ...
        try {
            await (0, index_1.createIflow)(testPackageId, testIflowId);
            iflowCreated = true;
            // Verify by trying to get it (or check list)
            const iflows = await (0, index_1.getAllIflowsByPackage)(testPackageId);
    ...
    it("should download the iflow folder", async () => {
        ...
        let iflowPath;
        try {
            iflowPath = await (0, index_1.getIflowFolder)(testIflowId);
            expect(iflowPath).toBeDefined();
            expect(typeof iflowPath).toBe('string');
            // Check if the directory exists
            const stats = await promises_1.default.stat(iflowPath);
            expect(stats.isDirectory()).toBe(true);
            // Optionally check for expected files within the downloaded structure (e.g., META-INF)
            await promises_1.default.access(path_1.default.join(iflowPath, 'META-INF', 'MANIFEST.MF'));
        }
        catch (error) {
            ...
            if (iflowPath) {
                try {
                    await promises_1.default.rm(iflowPath, { recursive: true, force: true });
                }
                catch (cleanupError) {
    ...
    it("should update an iflow with new file content", async () => {
        ...
        try {
            const updateResult = await (0, index_1.updateIflow)(testIflowId, [{
            ...
            const iflowPath = await (0, index_1.getIflowFolder)(testIflowId);
            const scriptPath = path_1.default.join(iflowPath, testFilePath);
            const actualContent = await promises_1.default.readFile(scriptPath, 'utf-8');
            ...
            try {
                const iflowPath = path_1.default.join(process.cwd(), 'temp', testIflowId); // Assuming temp folder structure
                await promises_1.default.rm(iflowPath, { recursive: true, force: true });
            }
            catch (cleanupError) { }
            throw error;
    ...
    it("should deploy an iflow and check status", async () => {
        ...
        try {
            const taskId = await (0, index_1.deployIflow)(testIflowId);
            expect(taskId).toBeDefined();
            expect(typeof taskId).toBe('string');
            console.log(`Deployment task ID for ${testIflowId}: ${taskId}. Waiting for status...`);
            const finalStatus = await (0, deployment_1.waitAndGetDeployStatus)(taskId);
    ...
    it("should retrieve endpoints for the deployed iflow", async () => {
        ...
        try {
            const endpoints = await (0, index_1.getEndpoints)(testIflowId);
    ...
    it("should retrieve configuration for the iflow (expecting empty)", async () => {
        expect(iflowCreated).toBe(true);
        try {
            const config = await (0, index_1.getIflowConfiguration)(testIflowId);
    ...
    it("should save the iflow as a new version", async () => {
        ...
        try {
            ...
            await (0, index_1.saveAsNewVersion)(testIflowId);
    ...
    it("should get the iflow content as a string", async () => {
        ...
        let iflowPath;
        try {
            const contentString = await (0, index_1.getIflowContentString)(testIflowId);
    ...
    it("should generate a diagram image for a specific iflow (if_simple_http_cld)", async () => {
        ...
        try {
            ...
            iflowPath = await (0, index_1.getIflowFolder)(targetIflowId);
            expect(iflowPath).toBeDefined();
            expect(typeof iflowPath).toBe('string');
            console.log(`Folder downloaded to: ${iflowPath}. Generating diagram...`);
            const imageBase64 = await (0, diagram_1.getiFlowToImage)(iflowPath);
...
//# sourceMappingURL=iflow.test.js.map