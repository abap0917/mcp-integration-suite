...
describe("API Destination Handling", () => {
    ...
    describe("getOAuthToken (Management API)", () => {
        it("should retrieve a valid OAuth token for Management API if OAuth env vars are set", async () => {
            ...
            try {
                const token = await (0, api_destination_1.getOAuthToken)();
                ...
                // Test caching
                const cachedToken = await (0, api_destination_1.getOAuthToken)();
        ...
        it("should throw an error if OAuth env vars are missing and it's called directly (though getCurrentDestination handles this)", async () => {
            ...
            await expect((0, api_destination_1.getOAuthToken)()).rejects.toThrow("API_OAUTH_TOKEN_URL environment variable is not set."); // Use exact string
        });
    });
    ...
    describe("getCurrentDestination", () => {
        it("should throw an error if API_BASE_URL is missing", async () => {
            ...
            await expect((0, api_destination_1.getCurrentDestination)()).rejects.toThrow("No API Url provided in project .env file");
        ...
        it("should throw an error if no complete authentication method (OAuth or Basic) is provided", async () => {
            ...
            await expect((0, api_destination_1.getCurrentDestination)()).rejects.toThrow("No Authentication method provided in project .env file");
            // Restore variables
            (0, dotenv_1.config)({ path: path_1.default.join(__1.projPath, '.env') });
        });
        ...
        it("should return OAuth destination if OAuth env vars are set", async () => {
            ...
            try {
                const destination = await (0, api_destination_1.getCurrentDestination)();
...
//# sourceMappingURL=api_destination.test.js.map