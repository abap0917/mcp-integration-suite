...
const sendMessageToCPI_1 = require("../messages/sendMessageToCPI");
const index_1 = require("../iflow/index"); // To potentially find a real endpoint
...
describe("Send Message to CPI API", () => {
    ...
    it("should attempt to send a GET request and handle the response status", async () => {
        ...
        try {
            const result = await (0, sendMessageToCPI_1.sendRequestToCPI)(testPath, method, contentType);
    ...
    it("should attempt to send a POST request and handle the response status", async () => {
        ...
        try {
            const result = await (0, sendMessageToCPI_1.sendRequestToCPI)(testPath, method, contentType, body);
            expect(result).toBeDefined();
            expect(result.status).toBeDefined();
            expect(typeof result.status).toBe('number');
    ...
    it.skip("should attempt to send a POST request to the echo mapping iflow (if available)", async () => {
        ...
        try {
            const result = await (0, sendMessageToCPI_1.sendRequestToCPI)(echoEndpointPath, method, contentType, body);
            expect(result).toBeDefined();
            expect(result.status).toBeDefined();
...
//# sourceMappingURL=sendMessageToCPI.test.js.map