...
const customErrHandler_1 = require("../customErrHandler");
const axios_1 = require("axios");
// Helper to create a mock AxiosError
const createMockAxiosError = (status, statusText, data, isResponseError = true // true for response error, false for request error
) => {
    const config = { headers: {} }; // Simplified config
    const request = isResponseError ? undefined : { path: '/test/path', method: 'GET' }; // Simplified request for non-response errors
    const response = isResponseError ? {
        data: data,
        status: status || 500,
    ...
    const error = new axios_1.AxiosError(`Request failed with status code ${status || 500}`, status ? String(status) : 'ERR_BAD_RESPONSE', config, request, response);
    return error;
};
describe('Custom Error Handler Utilities', () => {
    describe('extractAxiosError', () => {
        it('should format a standard Axios response error', () => {
            ...
            const result = (0, customErrHandler_1.extractAxiosError)(mockError);
        ...
        it('should format a null error', () => {
            const result = (0, customErrHandler_1.extractAxiosError)(null);
            expect(result.type).toEqual('text');
            if (result.type === 'text') {
    ...
    describe('formatError', () => {
        it('should call extractAxiosError and return its result', () => {
            ...
            const expectedResult = (0, customErrHandler_1.extractAxiosError)(mockError); // Get expected format
            const result = (0, customErrHandler_1.formatError)(mockError);
...
//# sourceMappingURL=customErrHandler.test.js.map