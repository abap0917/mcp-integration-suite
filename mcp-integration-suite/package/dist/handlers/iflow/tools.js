...
exports.registerIflowHandlers = exports.updateFiles = void 0;
const zod_1 = require("zod");
const iflow_1 = require("../../api/iflow");
const __1 = require("../..");
const diagram_1 = require("../../api/iflow/diagram");
const deployment_1 = require("../../api/deployment");
const customErrHandler_1 = require("../../utils/customErrHandler");
exports.updateFiles = zod_1.z.array(zod_1.z.object({
    filepath: zod_1.z
        .string()
        .describe('filepath within project. E.g. "resources/scenarioflows/integrationflow/myiflow.iflw    Does not have to be an existing file'),
    content: zod_1.z.string().describe(`File content.`),
    appendMode: zod_1.z
        .boolean()
        .default(false)
        .describe("This can be useful to split requests with large texts into smaller pieces.\n False: file will be replaced by given content True: existing file will be appended with content"),
}));
const registerIflowHandlers = (server) => {
    server.registerToolIntegrationSuite("get-iflow", `Get the data of an iflow and the contained ressources. 
Some ressources might relay on other package artefacts which are not included but reffrenced
`, {
        id: zod_1.z.string().describe("ID of the IFLOW"),
    }, async ({ id }) => {
        (0, __1.logInfo)(`trying to get iflow ${id}`);
        try {
            const fileContent = await (0, iflow_1.getIflowContentString)(id);
            return {
                content: [
                    {
                        type: "text",
    ...
    server.registerToolIntegrationSuite("create-empty-iflow", `Create an empty iflow without functionality. You probably want to add content to it afterwards with tool get-iflow and then update-iflow`, {
        packageId: zod_1.z.string().describe("Package ID"),
        id: zod_1.z.string().describe("ID/Name of the Iflow"),
    }, async ({ packageId, id }) => {
        try {
            await (0, iflow_1.createIflow)(packageId, id);
            return {
                content: [
                    {
                        type: "text",
    ...
    server.registerToolIntegrationSuite("update-iflow", `Update or create files/content of an iflow
...
src/main/resources/scenarioflows/integrationflow/<iflow id>.iflw contains the iflow in xml structure
        `, {
        id: zod_1.z.string().describe("ID of the IFLOW"),
        files: exports.updateFiles,
        autoDeploy: zod_1.z
            .boolean()
            .describe("True if iflow should be deployed after updateing, false if not"),
    }, async ({ id, files, autoDeploy }) => {
        (0, __1.logInfo)(`Updating iflow ${id} autodeploy: ${autoDeploy}`);
        try {
            const result = await (0, iflow_1.updateIflow)(id, files);
            (0, __1.logInfo)("Iflow updated successfully");
            if (autoDeploy) {
                (0, __1.logInfo)("auto deploy is activated");
                await (0, iflow_1.saveAsNewVersion)(id);
                const taskId = await (0, iflow_1.deployIflow)(id);
                if (taskId) {
    ...
    // Shit SAP API
    server.registerToolIntegrationSuite("get-iflow-endpoints", `
...
These are the prefixes based on protocol. So if you get /some/endpoint from get-iflow actual path is /http/some/endpoint for REST
    ...
    "SOAP": '/cfx/soapapi/'
        ...
        try {
            const endpoints = await (0, iflow_1.getEndpoints)(iflowId);
            return {
                content: [
                    {
                        type: "text",
    ...
    server.registerToolIntegrationSuite("iflow-image", "Get the iflow logic shown as a image/diagram", {
        iflowId: zod_1.z.string().describe("IFlow ID/Name"),
    }, async ({ iflowId }) => {
        const iflowPath = await (0, iflow_1.getIflowFolder)(iflowId);
        (0, __1.logInfo)(iflowPath);
        const pngString = await (0, diagram_1.getiFlowToImage)(iflowPath);
        return {
            content: [
                {
                    type: "image",
    ...
    server.registerToolIntegrationSuite("get-deploy-error", `
...
`, {
        id: zod_1.z
            .string()
            .describe("Artifact ID, can be iflow name or message mapping name etc."),
    }, async ({ id }) => {
        try {
            const errorReason = await (0, deployment_1.getDeploymentErrorReason)(id);
            return {
                content: [
                    {
                        type: "text",
    ...
    server.registerToolIntegrationSuite("deploy-iflow", `
deploy a iflow
If the deployment status is unsuccessful try getting information from get-deploy-error
		`, { iflowId: zod_1.z.string().describe("ID/Name of iflow") }, async ({ iflowId }) => {
        try {
            const taskId = await (0, iflow_1.deployIflow)(iflowId);
            const deployStatus = await (0, deployment_1.waitAndGetDeployStatus)(taskId);
            return {
                content: [
                    {
    ...
    server.registerToolIntegrationSuite("get-iflow-configurations", `Get all configurations of an IFlow
...
`, {
        iflowId: zod_1.z.string().describe("Id or name of the iflow"),
    }, async ({ iflowId }) => {
        try {
            const configurations = await (0, iflow_1.getIflowConfiguration)(iflowId);
            return {
                content: [
                    {
                        text: JSON.stringify({
    ...
    server.registerToolIntegrationSuite("get-all-iflows", `Get a list of all available iflows in a Package
If the user asks for all iflows, get all packages first and then query for each package`, {
        pkgId: zod_1.z.string().describe("Package id"),
    }, async ({ pkgId }) => {
        try {
            const allIFs = await (0, iflow_1.getAllIflowsByPackage)(pkgId);
            return {
                content: [
                    {
                        type: "text",
...
exports.registerIflowHandlers = registerIflowHandlers;
